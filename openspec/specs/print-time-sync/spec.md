# print-time-sync Specification

## Purpose
確保 Web API 回傳的 `estimatedPrintTime` 與 PRZ binary 內的列印時間來自**單一真值來源**：切片完成後以 PRZ 物理公式 `_compute_print_time()` 計算，並與 PRZ 下載端使用同一份持久化的 Mechado `prz_config` 與同一前處理，使網頁顯示值與實際 PRZ 列印時間一致。
## Requirements
...
- **THEN** 落檔前 SHALL 對該 config 套用 `_inject_retract_overrides()`（與下載端完全相同的前處理）
