# slicer-engine-deidentification Specification

## Purpose

後端切片引擎（Local agent CLI 子行程）與 Bundle-Launcher 正式包路徑之 **L1+L2 去識別**：改名重包（A）＋身分字串（B）＋精簡版 C′（可見性／strip／thread／Win export）；雙平台必過；不得以 C-full／OLLVM／packer／Crash-Reporter 攔截取代 A+B+C′。

**Promoted from：** `openspec/changes/backend-slicer-engine-deidentification`（task 8.5，2026-07-19）。  
**Acceptance evidence：** 同 change 之 `PROGRESS.md`／`evidence/`（Win 7.2／7.5／7.6；mac 7.1／7.4／7.6；Gate 5 Vance Approve）。
...
- **L2：** 同報告中使用者或工具可讀之堆疊／thread name／公開 export 不命中指紋黑名單（包含但不限於 `Slic3r::` 形式）。L2 **MUST** 以 [`design.md`](../../changes/backend-slicer-engine-deidentification/design.md) D3 定義之 **精簡版 C′**（strip＋thread＋Win export）達成；**MUST NOT** 要求全面 namespace 改名或 OLLVM。
...
正式包中的引擎執行檔與相關動態庫（Windows：exe／dll；macOS：CLI Mach-O）**MUST（必須）**使用產品簽核之內部名稱，**MUST NOT（不得）**使用 `PrusaSlicer`、`prusa-slicer` 或含對外品牌形式之 `prusa`／`slic3r` 檔名。Agent **MUST（必須）**能呼叫改名後路徑。正式包的目錄、symlink、loader error、資源檔名及使用者可見錯誤訊息亦 **MUST** 依 [`blacklist.md`](../../changes/backend-slicer-engine-deidentification/blacklist.md) 的 scope 判定。
...
正式對外引擎產物 **MUST（必須）** 以 **精簡版 C′** 達成 L2，使 [`acceptance-procedure.md`](../../changes/backend-slicer-engine-deidentification/acceptance-procedure.md) 定義之 macOS 與 Windows L2 表面 **MUST NOT（不得）** 命中 [`blacklist.md`](../../changes/backend-slicer-engine-deidentification/blacklist.md)。C′ **MUST** 包含：
...
3. **Thread name 去品牌（全部 call site）：** 所有 `set_current_thread_name`／`SetThreadDescription`（含 main 與 TBB／worker）**MUST** 改為簽核中性命名規則（見 [`naming-manifest.md`](../../changes/backend-slicer-engine-deidentification/naming-manifest.md)）。
4. **Windows 公開 export／DLL ABI 去品牌：** shim／DLL／export／VERSIONINFO／錯誤字串 **MUST** 原子遷移為簽核中性名並通過 smoke test。

L2 達標判定 **MUST** 綁定最終簽署包之靜態掃描，以及 release-equivalent qa（或可外部注入之 consumer）動態實測（token 掃描＋[`blacklist.md`](../../changes/backend-slicer-engine-deidentification/blacklist.md) §2.1 **第三方品牌歸因**正向複核）。C′ 為最低實作，**MUST NOT** 以「已實作 C′」預先宣稱達標。本版 **MUST NOT** 將全面 namespace／OLLVM／packer／`strings` 零品牌字串列為 L2 完成條件。內部符號組態 **MAY** 存在，但 **MUST NOT** 隨消費者正式包發布。
...
產品 owner **MUST（必須）** 在改名實作開始前簽核跨平台命名表（[`naming-manifest.md`](../../changes/backend-slicer-engine-deidentification/naming-manifest.md)）與機器可驗交接 schema（[`artifact-manifest.schema.md`](../../changes/backend-slicer-engine-deidentification/artifact-manifest.schema.md)），至少涵蓋 executable（含 macOS 真實 `OUTPUT_NAME`）、DLL、目錄、symlink、macOS `codeSigningID`／Info.plist 方針、VERSIONINFO、**全部 thread 命名規則**、**loader／export ABI**、resource 路徑、user-visible errors、pre／post_strip hash 與 flavor。法人 reverse-DNS App Bundle ID 與全面 C++ namespace 改名 **MUST NOT** 列為本版 L2 開工阻塞項。
