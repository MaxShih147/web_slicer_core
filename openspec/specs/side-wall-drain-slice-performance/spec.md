# side-wall-drain-slice-performance Specification

## Purpose

定義 `agent/ortho_pipeline.py::slice_mesh_at_z()`（Ortho pipeline Step 6，`generate_side_wall_drains()` 的內部輪廓切片函式）新增的三角形 Z min/max broad-phase 篩選（取代對全部三角形跑 scalar 迴圈找 Z 平面交點）必須維持的正確性契約：篩選條件與「至少存在一條端點嚴格分處切平面兩側的邊」的數學等價性（不得產生 false negative，但明確不等價於「一定產生 segment」）、篩選後 loop chaining 輸出與 `generate_side_wall_drains()`（Step 9/10 Boolean 之前）回傳值的 `np.array_equal` 等價判準、`generate_side_wall_drains()` 呼叫端管理 Z-bounds 生命週期（inner 不跨呼叫持有、outer 持有至函式返回，跨 retry 共用）不得使用全域或 Trimesh 快取，以及完整 pipeline 最終輸出（經 Step 9/10 Boolean 後）改用語意等價判準（faces/vertices count、volume、bounds、`is_watertight` 與固定 pre-change reference 一致）而非陣列相等的原因與範圍。
