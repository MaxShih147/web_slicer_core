...
## Purpose
定義 SLA 懸空角度門檻的語意與唯一執行位置：判定公式（`polar >= PI/2 + threshold`，等價於「表面斜度 ≤ 90 度 − 設定值」）與其方向語意、與 PhrozenOrca 刻度相反且僅於 45 度重合的刻意分歧、判定條件須為全代碼庫單一事實來源、角度過濾須於自動產點階段（`slaposSupportPoints`）執行且僅執行一次因而使匯出清單與實際長出的支撐柱在角度維度一致、角度與法線容差依支撐樹型的分派規則，以及邊界內縮點因法線平滑而產生的已知限制。
