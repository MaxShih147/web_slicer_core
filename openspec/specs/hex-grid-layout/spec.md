# hex-grid-layout Specification

## Purpose
定義 `HexGridLayout` 與 `compute_hex_grid_layout()` 的語意與行為——hex grid 幾何（中心座標、列數、行數、cell 間距）SHALL 依 `hollow_mesh.bounds` 計算並以單一 `HexGridLayout` 物件共用，使 `generate_hex_grid()` 與 `generate_drain_holes()` 在 ortho pipeline 中操作完全一致的 grid 座標系，確保所有 hex cell 壁面皆有對應的 drain hole 圓柱。亦定義無 `layout` 參數時的 fallback 行為，維持 standalone API endpoint 的向下相容性。

## Requirements

### Requirement: HexGridLayout 持有 grid 幾何的單一定義

`HexGridLayout` SHALL 持有以下欄位：`center_x`、`center_y`（grid XY 中心，mm）、`n_cols`、`n_rows`（grid 列數與行數）、`col_step`、`row_step`（cell 間距，mm）、`wall_thickness`（cell 壁面厚度，mm）。任何使用 `HexGridLayout` 的函式 SHALL 不得在函式內部另行重算上述欄位的等效值，而 SHALL 直接使用 layout 所提供的值。
