# job-file-reference-safety Specification

## Purpose
確保 `POST /slices/{job_id}/use-model-from/{source_job_id}` 在引用其他 job 的檔案時，無法被導向 job 儲存區以外的路徑。`source_job_id` 與 `source_file` 各自採用不同的檢驗機制：前者以字元集白名單攔下反斜線與點號，後者以「檔名 → 子目錄」對應表決定唯一的來源目錄，不再以存在性猜測退回其他目錄。
## Requirements
