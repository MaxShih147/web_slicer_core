# surgical-guide-vertex-weld-performance-decision Specification

## Purpose
記錄 Surgical Guide Auto Orient `_weld_and_build()` vertex quantization／weld 子階段（`agent/auto_orient_surg_guide.py` L227-243）效能調查的 `STOP_VERTEX_WELD` 決策：完整 core 理論改善上限、唯一已驗證公式（candidate 2）的預估收益與其驗證範圍限制、記憶體 dead-object 結構性估算、`del vmap` 解構成本描述，以及本決策與 edge-map `STOP_EDGE_MAP` 決策、既有四輪 Surgical Guide Auto Orient 效能優化之間互不影響的邊界。作為未來重新評估此子階段是否值得投入 production prototype 時的既定基準與觸發條件，防止已撤回的錯誤數值、已釐清的用詞或已劃定的範圍限制被誤引用。
## Requirements
