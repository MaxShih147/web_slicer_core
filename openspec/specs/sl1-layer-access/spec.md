# sl1-layer-access Specification

## Purpose
提供 .sl1 層檔的統一列舉與單層取用能力，作為層數統計、單層 PNG 取用與 PRZ 編碼的單一真值來源：以嚴格檔名過濾涵蓋 `.rle`（PRZ 快路徑）與 `.png` 兩種輸出、排除縮圖污染，並支援 RLE 單層即時解碼與失敗優雅降級（回 `None` → HTTP 404）。
