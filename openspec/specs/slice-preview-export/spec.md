...
定義預覽層圖封存（`*_preview.zip`）的產出契約：縮放比與消費端實際顯示需求的對應關係、降取樣濾波語意（box mean，並明確排除會讓細特徵視覺變粗的 max 濾波）、整數倍縮放比的快路徑及其與通用路徑逐位元組相同的要求、PNG 編碼的壓縮等級，以及「預覽產出失敗 MUST NOT 使切片失敗」的降級行為。

兩點需要在讀 requirement 之前知道：

- **預覽是輔助產物**。`.sl1` 的產出、`status.json` 的終態與 PRZ 下載能力都不得依賴預覽是否成功——一趟已完成的切片不應因為一個 UI 用的附屬檔案寫不出來而作廢。
- **現行縮放比為 `0.25`，而非依顯示需求反推出的 `0.10`**。降至 `0.10` 受一道跨 repo 硬閘門管制（DS-Online 需先移除以預覽圖生成列印用 PRZ 的降級路徑）。本規格刻意以現行值立約而非寫入目標值，因為 spec 的價值建立在「它描述的就是實際行為」之上；升級條件與屆時要補回的驗收場景記在對應的 requirement 內。
