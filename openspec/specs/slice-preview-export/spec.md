# slice-preview-export Specification

## Purpose

定義預覽層圖封存（`*_preview.zip`）的產出契約：縮放比與消費端實際顯示需求的對應關係、降取樣濾波語意（box mean，並明確排除會讓細特徵視覺變粗的 max 濾波）、整數倍縮放比的快路徑及其與通用路徑逐位元組相同的要求、PNG 編碼的壓縮等級、備援產線的層檔涵蓋範圍與已知差異，以及「預覽產出失敗 MUST NOT 使切片失敗」的降級行為。

四點需要在讀 requirement 之前知道：

- **預覽是輔助產物**。`.sl1` 的產出、`status.json` 的終態與 PRZ 下載能力都不得依賴預覽是否成功——一趟已完成的切片不應因為一個 UI 用的附屬檔案寫不出來而作廢。
- **縮放比不是常數，而是幅面長邊的函式**。顯示需求是一個絕對像素寬（約 1040 device px），縮放比卻是一個比例，因此任何單一固定值都會在機隊的兩端擇一犧牲：`0.25` 讓 15120 幅面過度取樣 3.6 倍，`0.10` 則讓 5760 / 3840 幅面掉到需求之下。縮放比改由量化函式決定，並吸附至引擎快路徑的 `1/N` 集合。
- **`N = 4`（即 `0.25`）同時是天花板**。長邊不足以達到目標寬度的機台一律停在此值，因此本能力的任何機台都不會比量化機制上線前更糊；代價是 3840 級機台維持 960 px，低於 1040 device px 的推導需求。這是明文接受的取捨，理由記在對應的 requirement 內。
- **有兩條產線，但只在縮放比與濾波上一致**。引擎產線（`--export-preview-pngs`）為常態路徑；Python 備援產線僅在 `model_preview.zip` 缺席時觸發。兩者的 ZIP 項目命名與影像編碼格式刻意不統一，該差異是契約的一部分而非缺陷，消費端須依實際項目名稱與副檔名解碼。

> 歷史註記：降低縮放比原受一道跨 repo 硬閘門管制——DS-Online 曾在 `downloadPrz` 失敗時拿預覽圖上採樣生成列印用 PRZ，降低縮放比會使上採樣倍率惡化。該降級路徑已由前端 change `remove-wasm-prz-fallback`（2026-08-07 封存）移除並改為明確拋錯，閘門解除，本能力才得以改採量化函式。
## Requirements
