...
**進度（2026-07-19 晚）：** Gate 0 大部完成；雙平台 PoC／C′ 已關；**Windows Launcher Gate 2／3＋手動 Authenticode Setup lifecycle 已落地**（≈83%）；macOS Launcher／AGPL／§7 動態仍開。詳見 [`PROGRESS.md`](./PROGRESS.md)。
...
- [x] 驗證 artifact manifest；僅複製 post-strip 產物（**Win 2026-07-19：** `build-windows-bundle.ps1`＋`scan_slicer_engine_windows.ps1`；macOS 待）
- [x] Windows layout 去品牌；agent 中立 env／路徑（macOS Launcher layout 待 Mac）
- [x] **驗證** strip／hash 後才允許簽署（不二次 strip／rename；**Win：** Authenticode **手動**，腳本內 skip）
- [ ] macOS codesign（中性 identifier）→ notarize → staple → Gatekeeper
- [x] Windows Authenticode → install/uninstall（**2026-07-19 手動 Setup Valid＋lifecycle smoke**；內嵌 app exe 未簽；見 `evidence/windows/launcher-1.0.0-postsign-20260719/`）
...
- [ ] macOS：正式 CI path、identity、**local＋global** 符號掃描通過（fork `scan_slicer_engine_macos.sh` 已有；Launcher CI 待）
...
- [x] scanner、blacklist **1.2**、pre／post_strip／**Setup post-sign** hashes 入 evidence（Win：[`evidence/windows/launcher-1.0.0-postsign-20260719/`](./evidence/windows/launcher-1.0.0-postsign-20260719/)）
- [x] 任一命中 fail-fast，無 `continue-on-error`（**Win Launcher／package 已 fail closed**；macOS Launcher 待）
...
- [ ] consumer 靜態 L1／L2＋binary inspection 通過（正式簽署包）
